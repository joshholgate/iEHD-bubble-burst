...
#define IMAX 200			/* number of cells in r-direction */
#define JMAX 400 			/* number of cells in z-direction */
#define INV_H 200.0			/* cell width */
//#define INV_RE 0.4 // for Li/W // 0.9 // for water, U=10m/s D=100um	/* inverse Reynolds number */
//#define INV_WE 140.0 // for W // 0.007 // for Li and water		/* inverse Weber number */
//#define INV_FRSQR 0.0			/* inverse square Froude number */
//#define INV_WEE 1600.0			/* inverse electrical Weber number */

#define INV_RE 0.1			/* inverse Reynolds number */
#define INV_WE 10.0			/* inverse Weber number */
#define INV_FRSQR 0.0			/* inverse square Froude number */
#define INV_WEE 42.15			/* inverse electrical Weber number */
#define APPL_E_WEIGHT 1.0		/* determines how much of E_0 corresponds to the applied field */
#define SELF_E_WEIGHT 0.0		/* and how much is due to charge on the droplet */
#define TOL 1e-6			/* successive-over-relaxation (SOR) error tolerance */
#define RELAX 1.9			/* relaxation parameter for SOR */
#define TIME_STEP 0.001			/* time between writing data to file */
#define FINAL_TIME 2.0			/* termination time of simulation */
	...
	for (int i=0; i<=IMAX+1; i++) {
		for (int j=0; j<=JMAX+1; j++) {
			grid->LS[i][j] = -( (double(j)-0.5)/INV_H - 1.0 )
						 + 0.025*cos(/*2.0*/3.14159*(double(i)-0.5)/INV_H);
		}
	}
/*	double R, Z, LSsurface, LSbubble;
	// Set up LS for 'bursting bubble' problem
	for (int i=0; i<=IMAX+1; i++) {
		for (int j=0; j<=JMAX+1; j++) {
			R = (double(i)-0.5)/INV_H;
			Z = (double(j)-0.5)/INV_H;
			LSsurface = -(Z-3.0);
			LSbubble = sqrt( (Z-2.25)*(Z-2.25)+R*R ) - 1.0;
			if ( LSsurface < 0.0 && LSbubble >= 0.0 ) {
				grid->LS[i][j] = LSsurface;
			} else if( LSsurface >= 0.0 && LSbubble < 0.0 ) {
				grid->LS[i][j] = LSbubble;
			} else {
				grid->LS[i][j] = min( LSsurface, LSbubble);
			}
		}
	}*/
#endif
...
/*  Define a function to write grid-centred values to file labelled with time t */
void write_macroscopic( Grid* grid, double t) {
...
int main() {
	...
	do {
		if ( t >= integer_t* TIME_STEP ) {
			write_macroscopic(&grid, integer_t);	// NB real time is t, integer_t will have rounding error
			if (int(integer_t)%10==0) write_grid(&grid, integer_t);
